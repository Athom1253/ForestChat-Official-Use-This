...
export function AnimatedBackground() {
  ...
  const customBgUrl = settings?.custom_background_url
  const bgType = settings?.background_type || 'animated'
  const customColor = settings?.custom_theme_color

  useEffect(() => {
    if (bgType === 'custom' && customBgUrl) {
      // Custom background image - no canvas animation needed
      return
    }
    ...
    const getParticleColor = (bg: string): string => {
      switch (bg) {
        ...
        case 'particles': return customColor || '#6366f1'
        case 'waves': return '#0ea5e9'
        case 'gradients': return customColor || '#a78bfa'
    ...
    const draw = () => {
      ...
      if (bg === 'gradients') {
        ...
        const c = customColor || '#6366f1'
        grad.addColorStop(0, `${c}14`)
        grad.addColorStop(0.5, `${c}0A`)
        grad.addColorStop(1, 'rgba(0, 0, 0, 0)')
        ctx.fillStyle = grad
        ctx.fillRect(0, 0, width, height)
      } else if (bg === 'waves') {
        const c = customColor || '#0ea5e9'
        ...
        particles.forEach((p) => {
          if (bg === 'rain') {
            ...
            if (p.y < 0 || p.y > height) p.vy *= -1
            const c = customColor || '#6366f1'
            ctx.fillStyle = `${c}${Math.floor(p.opacity * 255).toString(16).padStart(2, '0')}`
            ctx.beginPath()
            ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2)
            ctx.fill()
          }
  ...
  }, [bg, reduced, customBgUrl, bgType, customColor])

...
import { cn, timestampLabel, formatFileSize } from '@/lib/utils'
    ...
    <>
          ...
          {message.reactions.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1">
              {Object.entries(
                ...
                const tooltip = reacts.map((r) => {
                  const name = r.user_id === currentUserId
                    ? 'You'
                    : message.author && r.user_id === message.author.id
                      ? (message.author.display_name || message.author.username)
                      : r.user_id.slice(0, 8)
                  return `${name} · ${timestampLabel(r.created_at)}`
                }).join('\n')
                return (
                  <div key={emoji} className="relative group/reaction">
                    ...
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 bg-bg border border-border rounded-lg shadow-lg text-xs text-text whitespace-nowrap opacity-0 group-hover/reaction:opacity-100 transition-opacity pointer-events-none z-20" style={{ whiteSpace: 'pre-line' }}>
                      {tooltip}
                    </div>
                  </div>
                )
          ...
          <AnimatePresence>
            {showActions && (
              ...
              >
                ...
                {isOwn && (
                  <ActionButton onClick={() => setConfirmDelete(true)} title="Delete" danger>
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-[210] flex items-center justify-center p-4">
          ...
          <div className="relative bg-surface border border-border rounded-2xl shadow-2xl max-w-sm w-full p-6">
            <h3 className="text-lg font-semibold text-text mb-2">Delete message?</h3>
            <p className="text-sm text-text-muted mb-6">This message will be permanently deleted.</p>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setConfirmDelete(false)} className="px-4 py-2 text-sm font-medium text-text-muted hover:text-text bg-surface-hover hover:bg-border rounded-lg transition-colors">Cancel</button>
              <button onClick={handleDelete} className="px-4 py-2 text-sm font-medium text-white bg-error hover:bg-red-600 rounded-lg transition-colors">Delete</button>
            </div>
          </div>
        </div>
      )}

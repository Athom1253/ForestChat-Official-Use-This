...
export function ActiveCallModal({ callType, channelName, isIncoming, onEnd }: ActiveCallModalProps) {
  const [micEnabled, setMicEnabled] = useState(true)
  const [cameraEnabled, setCameraEnabled] = useState(callType === 'video')
  const [screenSharing, setScreenSharing] = useState(callType === 'screen')
  ...
  const typeLabel = callType === 'voice' ? 'Voice Call' : callType === 'video' ? 'Video Call' : 'Screen Share'
  ...
  useEffect(() => {
    if (cameraEnabled && (callType === 'video' || screenSharing)) {
      setTimeout(() => attachStreamToVideo(), 50)
    }
  }, [cameraEnabled, screenSharing, attachStreamToVideo, callType])

  const showVideo = (callType === 'video' || screenSharing) && cameraEnabled
    ...
    <div className="fixed inset-0 bg-black/90 z-[200] flex flex-col items-center justify-between p-6">
      ...
      <div className="w-full flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {isIncoming && <span className="text-white/30 text-xs">(incoming)</span>}
        </div>
      ...
      <div className="flex-1 flex items-center justify-center w-full">
        {error ? (
          <div className="text-center max-w-md">
            ...
            <button onClick={endCall} className="btn-ghost text-white">Close</button>
          </div>
        ) : showVideo ? (
          <video
            ref={localVideoRef}
            ...
            style={{ transform: callType === 'video' && !screenSharing ? 'scaleX(-1)' : 'none' }}
          />
        ) : (
          <div className="text-center">
            ...
            {connected && <p className="text-white/40 text-sm mt-1">{micEnabled ? 'Mic on' : 'Mic muted'}</p>}
          </div>
        )}
      </div>

...
export function PinnedMessagesPanel({ channelId, onClose }: PinnedMessagesPanelProps) {
  ...
  async function loadPinnedMessages() {
    ...
    if (!error && data) {
      setPinnedMessages(data as (Message & { author: Profile | null })[])
    }
  }

  const unpinMessage = async (msgId: string) => {
    const { error } = await supabase.from('messages').update({ is_pinned: false }).eq('id', msgId)
    if (error) return
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        {pinnedMessages.length === 0 ? (
          <div className="text-center py-8 text-text-muted">
            ...
            <p className="text-xs mt-1">Pin a message from its context menu</p>
          </div>
        ) : (
          pinnedMessages.map((msg) => (
            <div key={msg.id} className="p-3 bg-bg rounded-lg group relative">
              ...
              <button
                ...
                className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-text-muted hover:text-error transition-all"
                title="Unpin"
              >

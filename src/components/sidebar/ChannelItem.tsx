...
export function ChannelItem({ channel, onUpdated }: ChannelItemProps) {
  ...
  const isActive = loc.pathname === '/' && new URLSearchParams(loc.search).get('c') === channel.id
  ...
  const leaveChannel = async () => {
    if (!channel.member) return
    const { error } = await supabase.from('channel_members').delete().eq('id', channel.member.id)
    if (error) { toast.error('Failed to leave channel'); return }

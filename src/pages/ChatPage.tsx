...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const sub = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'message_reactions' },
        (payload) => {
          const reaction = payload.new as Reaction | null
  ...
  async function loadMessages() {
    ...
    setLoading(true)
    debug.log('messages', 'Loading messages for channel', channelId)
    ...
    if (error) {
      debug.log('messages', 'Load error', error)
      toast.error('Failed to load messages: ' + error.message)
      setLoading(false)
      return
    }

    debug.log('messages', 'Loaded', { count: data?.length || 0 })
  ...
  async function markAsRead() {
    ...
    await supabase.from('read_receipts').upsert({
      chat_id: channelId, user_id: user.id,
  ...
  return (
    <div className="flex-1 flex flex-col page-bg min-w-0 h-full">
      <ChatTopBar
        channel={channel}
      ...
      <div className="flex-1 flex min-h-0 relative">
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0">
          <div
            ...
            className="flex-1 overflow-y-auto overflow-x-hidden px-4 py-4 space-y-1 min-h-0"
          >
            {messages.length === 0 ? (
          ...
          <MessageInput
            ...
            onSent={() => setTimeout(() => scrollToBottom(false), 200)}
          />
        </div>

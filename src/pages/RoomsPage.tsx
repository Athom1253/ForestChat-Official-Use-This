...
export default function RoomsPage() {
  ...
  async function loadChannels() {
    ...
    if (error) { debug.log('rooms', 'Load failed', error) }
  ...
  const createRoom = async () => {
    ...
    debug.log('rooms', 'Creating room', { name: newRoomName.trim(), code })
    ...
    if (error) {
      debug.log('rooms', 'Create room failed', error)
      toast.error('Failed to create room: ' + error.message)
      return
    }

    debug.log('rooms', 'Room created, adding membership', { id: data.id })
    const { error: memberError } = await supabase.from('chat_memberships').insert({
      chat_id: data.id,
      user_id: user.id,
      role: 'owner',
    })
    ...
    if (memberError) {
      debug.log('rooms', 'Membership insert failed', memberError)
      toast.error('Room created but failed to add you as member: ' + memberError.message)
    } else {
      debug.log('rooms', 'Membership added successfully')
    }
  ...
  const joinRoom = async () => {
    ...
    if (memberError) { toast.error('Failed to join room: ' + memberError.message); return }

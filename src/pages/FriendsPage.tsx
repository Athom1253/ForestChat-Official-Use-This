...
export default function FriendsPage() {
  ...
  const startDM = async (friendUserId: string) => {
    ...
    const { data: blocked } = await supabase
      ...
      .maybeSingle()
    if (blocked) { toast.error('Cannot message this user'); return }
    ...
    const { data: channel, error: chError } = await supabase.from('chats').insert({
      type: 'dm',
      created_by: user.id,
    }).select().single()

    if (chError) { toast.error('Failed to create DM: ' + chError.message); return }

    const { error: memberError } = await supabase.from('chat_memberships').insert([
      { chat_id: channel.id, user_id: user.id, role: 'member' },
      { chat_id: channel.id, user_id: friendUserId, role: 'member' },
    ])

...
import { useDebugStore, debug } from '@/stores/debug'
import { AlertHistoryPanel } from '@/components/ui/ToastContainer'
...
export default function AppLayout() {
  ...
  const { logs: debugLogs, enabled: debugEnabled, toggle: toggleDebug, clear: clearDebug } = useDebugStore()
  ...
  async function loadChannels() {
    if (!user) return
    try {
      debug.log('channels', 'Loading channels for user', user.id)
      ...
      debug.log('channels', 'Memberships found', { count: members?.length || 0 })
      ...
      debug.log('channels', 'Failed to load channels', err)
      ...
      <div className="w-16 flex-shrink-0 bg-surface border-r border-border flex flex-col items-center py-3 gap-2">
        ...
        <button
          onClick={() => {
            ...
            if (newState) useDebugStore.getState().log('debug', 'Debug console enabled')
          }}
          className={cn('w-11 h-11 flex items-center justify-center rounded-xl transition-all hover:bg-surface-hover', debugEnabled && 'bg-warning/20')}
          title="Debug"
        >
          <svg className={cn('w-5 h-5', debugEnabled ? 'text-warning' : 'text-text-muted')} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {location.pathname === '/' && (
        <div className={cn(
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-4">
            {/* Alert History at top of sidebar */}
            <AlertHistoryPanel />
            {loading ? (
      ...
      <AnimatePresence>
        {debugEnabled && showDebug && (
          <motion.div
            ...
            className="fixed bottom-4 right-4 z-[250] w-96 max-h-[60vh] bg-surface border border-border rounded-xl shadow-2xl flex flex-col"
          >
            <div className="flex items-center justify-between p-3 border-b border-border">
              <h3 className="text-sm font-semibold text-text">Debug Console</h3>
              <div className="flex gap-2">
                <button onClick={clearDebug} className="text-xs text-text-muted hover:text-error">Clear</button>
                <button onClick={() => setShowDebug(false)} className="text-text-muted hover:text-text">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 font-mono text-xs">
              {debugLogs.length === 0 ? (
                <p className="text-text-muted text-center py-4">No debug logs yet. Enable debug and interact with the app.</p>
              ) : debugLogs.map((log) => (
                <div key={log.id} className="p-2 rounded bg-bg">

...
// Admin tables may use different names than the compat views
const ADMIN_LOGS_TABLE = 'admin_audit_log'
...
export default function AdminPage() {
  ...
  async function loadAll() {
    const [{ data: usersData }, { data: invitesData }, { data: reportsData }, { data: logsData }] = await Promise.all([
      ...
      supabase.from(ADMIN_REPORTS_TABLE).select('*').order('created_at', { ascending: false }),
      supabase.from(ADMIN_LOGS_TABLE).select('*').order('created_at', { ascending: false }).limit(50),
    ])
    ...
    const { count: totalChannels } = await supabase.from('chats').select('*', { count: 'exact', head: true })
  ...
  const toggleAdmin = async (p: Profile) => {
    await supabase.from('app_users').update({ is_admin: !p.is_admin }).eq('id', p.id)
  ...
  const resolveReport = async (id: string, status: 'resolved' | 'dismissed') => {
    const { error } = await supabase.from(ADMIN_REPORTS_TABLE).update({
      status,
